...
This model predicts only the Euclidean distance from origin (not direction),
using only absorption time features (no impact angles).

Architecture:
    Input: 20 time-based features
    → BatchNorm → Linear(20→64) → LeakyReLU → Dropout
    → Linear(64→32) → LeakyReLU → Dropout
    → Linear(32→1) → distance
...
class TimeFeatureExtractor:
    ...
    No angle information used - for distance-only prediction.
    """
    ...
    def fit(self, samples):
        ...
        # Calibrate time → distance relationship (diffusion theory)
        times_median = []
        distances = []
        for s in samples:
            if len(s['absorption_times']) > 0:
...
class DistanceMLPModel:
    """
    Complete model for distance prediction.
    
    Input: Absorption times only (no angles)
    ...
    def __init__(self, hidden_dims=[64, 32], dropout=0.3, n_molecules=2000):
        ...
        self.network = DistanceMLP(
            input_dim=20,
            hidden_dims=hidden_dims,
            dropout=dropout
        )
        # Distance range: sqrt(15²+15²) ≈ 21 to sqrt(90²+90²) ≈ 127
        self.dist_min = 20
        self.dist_max = 130
