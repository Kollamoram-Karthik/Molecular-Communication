...
This model uses hand-crafted physics-informed features extracted from
absorption times and impact angles, then applies an MLP to predict (x0, y0).

Architecture:
    Input: 35 physics-based features
    → BatchNorm → Linear(35→128) → LeakyReLU → Dropout
    → Linear(128→128) → LeakyReLU → Dropout + Residual
...
Features include:
    ...
    - Cross features: time-angle correlation, angle drift
    - Physics estimates: calibrated distance, direction
"""
...
class FeatureMLP(nn.Module):
    """MLP network with residual connections for position prediction."""
...
class FeatureMLPModel:
    """
    Complete model combining physics-based feature extraction with MLP.
    
    This is the wrapper class that handles:
    - Feature extraction from raw absorption data
    - Target normalization/denormalization
    - Physics-only baseline estimation
    """
    ...
    def fit_physics(self, samples):
        """Fit the physics-based calibration on training data"""
    ...
    def denormalize_target(self, pred):
        """Convert normalized predictions back to original scale (μm)"""
    ...
    def get_physics_estimate(self, sample):
        """Get raw physics-based estimate (baseline)"""
    ...
    def predict(self, sample):
        ...
        with torch.no_grad():
            ...
            pred_normalized = self.network(features_tensor).numpy()[0]
